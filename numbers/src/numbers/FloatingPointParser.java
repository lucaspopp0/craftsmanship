...
public class FloatingPointParser {
	...
	private DecimalInput getExpo() {
		return expo.orElseGet(() -> DEFAULT_EXPONENT);
	}
	...
	/*
	 * A floating point constant is valid if it contains one or two floating chunks and both the base and the integer exponent are valid.
	 */
	...
	class TestHook {
		...
		boolean containsAtLeaftOneFloatingChunk(String number) {
			return FloatingPointParser.build(number).containsAtLeastOneFloatingChunk();
		}
		
		boolean hasValidBase(String number) {
			return FloatingPointParser.build(number).hasValidBase();
		}
		
		boolean hasValidIntegerExponent(String number) {
			return FloatingPointParser.build(number).hasValidIntegerExponent();
		}
